...
namespace MyNameSpace
{
	...
	class Program
	{
		...
		static void Main()
		{
			...
			Console.WriteLine($"Full Namespace: {myClassType.Namespace}");
			...
			foreach (var property in myClassType.GetProperties())
			{
				Console.WriteLine($"Property Name: {property.Name}, Type: {property.PropertyType}, " +
					              $"DeclaringType: {property.DeclaringType}, CanRead: {property.CanRead}, CanWrite: {property.CanWrite}");
			}
			...
			{
				Console.WriteLine($"\t{method.ReturnType} {method.Name} {method.IsPublic}({GetParameterList(method.GetParameters())})");
			}
			...
			object[] parameters = { 33, "Oussama Souissi" };
